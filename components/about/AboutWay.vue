...
<template>
  <div class="pb-[3rem] lg:pb-[8.75rem]">
    <h2 class="text-system-black-900 text-[1.375rem] pb-[1.375rem] lg:pb-[4rem] lg:text-[2.25rem] font-bold">От идеи до
      реализации</h2>
    <div ref="container"
      class="grid grid-cols-1 aspect-[105/25] about-way-container lg:grid-cols-3 gap-[3rem] lg:gap-x-[1.88rem] lg:gap-y-[2.5rem]">

      <transition enter-active-class="transition duration-1000 ease-out" enter-from-class="-translate-y-4 opacity-0"
        ...
        <figure v-if="firstVisible" class="flex about-way-item flex-col gap-[1.135rem] lg:gap-[1.7rem]">
          <NuxtImg :src="list[0]?.data.file.fileId" provider="directus"
            class="aspect-[35/24] object-cover rounded-[0.635rem]" />
          <figcaption class="font-bold text-center">{{ list[0]?.data.caption }}</figcaption>
        </figure>
      </transition>
      <transition enter-active-class="transition duration-1000 delay-500 ease-out"
      ...
      <transition enter-active-class="transition duration-1000 delay-1000 ease-out"
        enter-from-class="-translate-y-4 opacity-0" enter-to-class="translate-y-0 opacity-100"
        leave-active-class="transition duration-150 ease-in" leave-from-class="translate-y-0 opacity-100"
        leave-to-class="-translate-y-4 opacity-0">
