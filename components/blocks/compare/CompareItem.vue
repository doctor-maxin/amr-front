    ...
    <div class="flex gap-4 max-w-[35rem] lg:gap-[3.8rem] flex-col">
        ...
        <div>
            ...
            <div class="flex flex-col gap-5">
                <div v-for="option of product.optionsValues"
                    class="flex text-sm lg:flex-row lg:text-base gap-1 flex-col w-full justify-between">
                    <div class="lg:opacity-100 font-medium " :class="{
                        'font-semibold text-accent-300': uniquePropIds.includes(option.optionValues_id.parentId.id)
                    }">
                        {{ option.optionValues_id.parentId.name }}
                    </div>
                    <div class="font-semibold">{{ option.optionValues_id.value }}</div>
                </div>
            </div>
        </div>
        <div class="w-full gap-3 flex">
            <div
                ...
                <nuxt-link class="text-xl hover:text-white" :to="`/catalog${product.handle}`"> <span>{{ product.name
                }}</span></nuxt-link>
                <span class="whitespace-nowrap">{{ product.canNotBye ? 'от' : '' }} {{ toMoney(product.price) }}</span>
            </div>
            <button @click="compareStore.removeFromCompare(product.id)"
                class="rounded-[1rem] items-center justify-center backdrop-blur-[10px] bg-system-black-600 flex text-white h-full p-5"><svgo-close
                    filled class="text-2xl" /> </button>
            <button v-if="!product.canNotBye && product.count > 0" @click="cartStore.addProduct(product.id)"
                class="rounded-[1rem] items-center justify-center backdrop-blur-[10px] bg-system-black-600 flex gap-1 text-white p-5 h-full"><svgo-cart
                    filled class="text-2xl" />
