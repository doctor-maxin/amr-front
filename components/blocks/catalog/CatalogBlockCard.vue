...
<template>
	<div class="rounded-[1.25rem] overflow-hidden">
		<nuxt-link :to="`/catalog${item.handle}`">
			<NuxtImg provider="directus" :src="item.image" class="aspect-[422/648] w-full grayscale-[50%] object-cover"/>
		</nuxt-link>
		<div class="absolute bottom-3 lg:bottom-5 w-full px-3 lg:px-5">
			<div
				...
				<span class="font-bold leading-[140%] text-lg">{{ item.name }}</span>
				<nuxt-link :to="`/catalog${item.handle}`"
			...
			>
				Посмореть
					...
					<svgo-arrow-top-right
						class="text-2xl icon  transition-colors" filled/>
				</nuxt-link>
			</div>
		</div>
	</div>
