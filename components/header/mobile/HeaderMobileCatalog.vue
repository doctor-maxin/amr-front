...
<template>
	<div class="relative">
		<div :class="{
			...
			<template v-for="item of mainCategories" :key="item.id">
				<button class="text-xl text-system-gray-800 font-semibold flex justify-between w-full"
					...
					<nuxt-link @click.native="api.close()" :to="`/catalog${item.handle}`">{{ item.name }}</nuxt-link>
		...
		<transition enter-active-class="transition duration-200 ease-out" enter-from-class="translate-x-full opacity-0"
			...
			<div v-if="selectedCategory" :class="{}" class="flex w-full absolute left-0 transition-transform flex-col">
				...
				<div class="flex flex-col gap-[1.135rem]">
					<template v-for="item of subCategories" :key="item.id">
						<nuxt-link :to="`/catalog${item.handle ?? ''}`" class="cursor-pointer hover:underline"
							@click.native="api.close()"><span class="text-system-black-900 font-medium">{{
								item.name
							}}</span></nuxt-link>
					</template>
				</div>
