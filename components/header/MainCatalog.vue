...
const mainCategories = computed(() => categories.value?.filter((item: any) => !item.parentId))
const selectedCategory = ref(mainCategories.value?.find((item: any) => item.childrens.length))
const subCategories = computed(() => categories.value?.filter((item: any) => selectedCategory.value?.childrens.includes(item.id)))
const setOffsets = () => {
		...
		gridTemplateColumns: `${leftOffset}px 1fr`
			...
			<div
				...
				<template v-for="item of subCategories">
					<nuxt-link @click.native="api.close()" :to="`/catalog${item.handle.toLowerCase()}`" class="main-catalog-link cursor-pointer hover:underline"><span
