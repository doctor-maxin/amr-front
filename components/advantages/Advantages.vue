...
<template>
	<section class="advantages lg:px-[4.37rem] overflow-x-clip">
		...
		<suspense>
			<div
				class="grid sm:grid-cols-2 items-start gap-9 px-4 lg:px-0 lg:gap-x-[6rem] lg:gap-y-0"
			>
				<template v-for="(item, index) of list" :key="item?.id">
					<article
						v-if="item"
						class="flex flex-col relative advantage-item"
					>
						<div
							class="relative lg:aspect-auto lg:max-w-[35rem] lg:max-h-[34rem]"
							:class="{
								'lg:mt-8': index % 2 === 0,
						...
						>
							...
							>
								...
								<p
									class="leading-[140%] text-system-gray-900 text-sm"
								>
									{{ item.description }}
								</p>
							</div>
