...
<template>
	<div class="bg-system-gray-500 relative page-header overflow-hidden">
		<div
			class="flex flex-col p-7 items-center lg:pt-[1.62rem] lg:pb-[2.5rem] gap-1 lg:gap-0 z-10 text-white bg-gradient-to-b from-black-30 to-black-30 w-full h-full">
			...
			<button v-if="link"
				...
				<span class="group-hover:text-accent-300 transition-colors">{{
					link.title
				}}</span>
			</button>
			...
			<button v-if="to"
				...
				type="button" @click="router.push(to.path)">
				<span class="group-hover:text-accent-300 transition-colors">{{
					to.title
				}}</span>
		...
		<NuxtImg :src="image" class="absolute left-0 top-0 w-full max-h-full object-cover object-center"
			provider="directus" />
	</div>
</template>
