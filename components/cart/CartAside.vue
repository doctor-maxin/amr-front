	...
	fetch('/api/cart/check', {
		...
		if (!response.ok) {
			...
			for (const id of result) {
				const {setErrors} = useField(`product-${id}`)
				setErrors(appConfig.messages.productSoldOut)
			}
		} else {
...
<aside class="rounded-[1.25rem]  flex-1 text-system-gray-800 bg-system-gray-500 px-[1.135rem] pt-[1.69rem] pb-[2.375rem] flex flex-col gap-3">
	<div class="flex justify-between gap-2 lg:text-lg">
		...
		<span class=" text-right font-bold whitespace-nowrap">{{toMoney(sum)}}</span>
	</div>
	<div v-if="delivery.calculated" class="flex justify-between gap-2 lg:text-lg">
		<span class="font-medium">Доставка</span>
		<span class="text-right font-bold whitespace-nowrap">{{toMoney(delivery.amount)}}</span>
	</div>
	<div v-if="discount.promoCode" class="flex justify-between gap-2 lg:text-lg">
		<span class="font-medium">Скидка</span>
		<span class="text-right font-bold whitespace-nowrap">{{toMoney(discount.amount)}}</span>
	</div>
	<div class="flex lg:mt-4 justify-between lg:text-xl gap-2">
		<span class="font-bold ">Итого</span>
		<span class="text-right font-bold whitespace-nowrap">{{toMoney(total)}}</span>
	</div>
	<UiButton @click="toCheckout" class="fixed lg:mt-2 lg:static bottom-4 z-20 left-4 w-[calc(100%_-_2rem)] lg:w-full" type="button" variant="dark" title="Перейти к оформлению" title-class="!text-base" />
</aside>
</template>
