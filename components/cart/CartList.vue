	...
	<div class="max-w-[64.125rem] w-full">
		...
		<div class="lg:py-8 lg:px-6 gap-8 flex flex-col">
			<template v-for="item of lines.values()" :key="item.id">
				...
				>
					<nuxt-link
						:to="`/catalog${item.product.handle ?? ''}`"
					...
					>
						...
						/>
						<div
							class="flex flex-col gap-1 font-semibold text-system-black-900"
						>
					...
					<div
						class="row-start-2 lg:row-start-1 lg:col-start-2 col-start-1"
					>
						<UiPlusMinusInput :line="item" />
					</div>
					<div class="hidden lg:block font-semibold text-xl">
						{{ toMoney(item.product.price) }}
					</div>
					<div
						class="row-start-2 col-start-3 justify-self-end lg:row-start-1 lg:col-start-4"
					>
						<button
						...
						>
							<svgo-close
								class="text-2xl transition-colors !mb-0 group-hover:lg:text-accent-300"
								filled
							/>
							<span
								class="text-system-gray-800 font-medium transition-colors group-hover:lg:text-accent-300 lg:font-semibold"
								>Удалить</span
							>
						</button>
					</div>
