...
<template>
	<div class="fixed z-10 w-full left-0 bottom-4 px-4 lg:static lg:px-0 lg:sticky lg:bottom-[4rem]" itemprop="offers"
		...
		<div
			...
			<div class="flex flex-col">
				<span class="text-lg font-bold" itemprop="price">{{ product.canNotBye ? "от " : ""
			...
			<button @click="addProductToFavorites(product.id)"
				...
				type="button">
				<svgo-heart class="text-2xl" filled :class="{
					'fill-system-black-950': hasInFavorites(product.id),
				}" />
			</button>
			...
			<button v-if="!product.canNotBye" :disabled="product.count <= 0" @click="addProductItem(product.id)"
