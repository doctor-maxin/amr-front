...
<template>
  ...
  <div v-else>
    <transition name="fade">
      <div v-if="!productStore.isLoading && productStore.data"
        class="py-12 flex flex-col gap-16 lg:gap-14 lg:pb-8 lg:pt-0 lg:-mt-5 max-w-full">
        <div v-for="item in productStore.data.materials">
          ...
          <div class="flex gap-4 p-1 overflow-x-auto snap-mandatory snap-x snap-start">
            <ProductInfoOptionItem v-for="material of item.productConstructorMaterials_id.items"
              :material="material.productConstructorMaterialItem_id" :material-group="item.productConstructorMaterials_id"
