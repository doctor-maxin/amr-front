...
import { useProductStore, type IMaterialItem, type IProductMaterial } from "~/store/product.store";
...
<template>
  <div class="flex flex-col gap-3">
    <button @click="productStore.selectOption(materialGroup.id, material.id)"
      ...
      <NuxtImg provider="directus" :src="material.image" width="160" height="160" format="webp" loading="lazy" placeholder
        class=" w-[5.1875rem] h-[5.1875rem] aspect-square lg:h-[10rem] lg:w-[10rem] " />
    </button>
    <span class="flex items-start flex-col gap-1">
      <span class="hidden lg:block text-base font-medium">{{ material.name }}</span>
