...
<template>
  ...
  >
    <div class="flex justify-between max-w-[54rem] gap-2 lg:gap-4 items-center flex-wrap">
      <div
          class="flex lg:rounded-[5rem] lg:w-fit gap-4 lg:bg-system-gray lg:gap-1 lg:p-[0.31rem]"
      >
        <button
            :class="{
        ...
        <button
            ...
            class="text-lg lg:rounded-[5rem] lg:text-base lg:text-opacity-100 lg:py-3 lg:px-[1.385rem] transition-colors lg:font-semibold font-bold text-system-gray-800"
            type="button"
            @click="productStore.setTab(Tabs.PROPS)"
        >
          Характеристики
        </button>
      </div>
    ...
    <main class="flex flex-1 flex-col max-w-[54rem]">
      ...
      <ProductInfoProperties
          v-else-if="activeTab === Tabs.PROPS"
          :product="product"
          :variant="variant"
      />
      <ProductInfoOptions v-if="activeTab === Tabs.CONST"
                          :product="product"
                          :variant="variant"
      />
      <div class="lg:hidden flex flex-col gap-4">
        ...
        <UiButton
            ...
            class="w-full h-[3.25rem]" title="Покупка в 1 клик" title-class="text-center block mx-auto" type="button"/>
      </div>
      <ProductInfoOptions v-if="activeTab === Tabs.CONST"
                          :product="product"
                          :variant="variant"
