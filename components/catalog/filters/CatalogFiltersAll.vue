...
<template>
	<transition enter-active-class="transition duration-150 ease-out" enter-from-class="-translate-y-4 opacity-0"
		...
		<div v-if="api.isOpen"
			class="z-20 pt-8 absolute -right-4 lg:right-0 top-[calc(100%_-_1rem)] lg:top-[calc(100%_+_1rem)] lg:pt-8 lg:pl-[2.63rem] lg:pr-[1.0625rem] pb-5 lg:pb-[2.875rem] w-screen lg:max-w-[26rem] rounded-[1.25rem] bg-white lg:bg-system-gray pl-4 pr-[1.0625rem]">
			<div v-bind="api.contentProps" class="max-w-full">
				<span
					class="hidden lg:block font-semibold text-[1.375rem] text-system-black-900 mb-[1.37rem]">Фильтры</span>
				<div class="max-h-[46rem] overflow-y-auto scrollbar flex flex-col pr-4 gap-5"
					v-bind="accardionApi.rootProps">
					<div v-for="item of filteredItems" :key="item.options_id?.id" class="bg-system-gray-600 rounded-xl"
						v-bind="accardionApi.getItemProps({
							value: item.options_id?.id,
						})
							">
						<button v-bind="accardionApi.getItemTriggerProps({
							value: item.options_id?.id,
						})
						...
						<transition>
							<div v-if="!accardionApi.getItemContentProps({
								value: item.options_id?.id,
							})?.hidden
								" v-bind="accardionApi.getItemContentProps({
		value: item.options_id?.id,
	})
		" class="flex flex-col gap-2 pb-[1.375rem]">
								<label v-for="val of item.options_id?.values" :key="val.id"
									class="flex gap-3 items-center px-5 cursor-pointer">
									<vee-field type="checkbox" class="filter-input" :name="item.options_id?.id"
										:value="val.id" />
				...
				<div class="pt-5 pr-5">
					<UiButton variant="dark" type="submit" title="Применить" class="w-full mb-4" @click="applyFilters" />
					<UiButton class="mb-4 lg:hidden w-full" title="Сбросить фильтры" title-class="text-center w-full"
						@click="useEvent('reset-filters')" />
				</div>
			</div>
		</div>
	</transition>
