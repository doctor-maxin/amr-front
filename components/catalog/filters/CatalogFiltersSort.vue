...
watch(
	() => props.api.isOpen,
	() => {
		if (props.api.isOpen) apiSelect.value.open();
		else apiSelect.value.close();
	},
	{
		immediate: true,
	}
...
const updateParam = () => {
	...
	props.api.close();
	useEvent("on-sort", apiSelect.value.value.toString());
};

const onSortChange = (item) => {
	apiSelect.value.setValue([item.value]);
	if (!isTablet.value) {
		useEvent("on-sort", item.value);
