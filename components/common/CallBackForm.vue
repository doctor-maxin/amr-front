	...
	<section class="grid grid-cols-1 lg:grid-cols-3" ref="container">
		<div class="flex flex-col order-1 relative">
			<img
				...
				src="/images/callback-bg.png"
				alt="Мебель"
			/>
			<div
				class="px-4 lg:p-[4.38rem] lg:pb-[5rem] h-full lg:flex lg:flex-col pt-[3.19rem] pb-[2.59rem]"
			>
				<h4
					class="mb-1 text-xs lg:text-sm font-bold text-system-gray-500"
				>
					КАТАЛОГ
				</h4>
				<h3
					class="text-white lg:text-[2.25rem] font-bold mb-[1.62rem] text-[1.375rem]"
				>
		...
		>
			<div class="lg:flex hidden gap-6 flex-col items-start">
				...
				<button
					...
					type="button"
					@click="setCallBackType('calc')"
				>
					Получить расчет стоимости
				</button>
			</div>
			<div
				class="bg-system-gray-600 lg:mt-auto flex flex-col rounded-[1.25rem] px-[1.635rem] py-5 mt-6"
			>
				<h4 class="font-bold text-xl mb-2.5">Свяжитесь с нами</h4>
				<TelLink
					:tel="settings?.phone"
					class="text-base lg:text-base mb-2"
				/>
				<EmailLink
					:email="settings?.Email"
					class="text-base lg:text-base"
				/>
		...
		<div
			class="bg-system-gray-500 order-2 lg:order-3 px-4 pt-[2.875rem] lg:py-[5rem] pb-[1.635rem]"
		>
			...
			<div
				class="flex lg:max-w-[26.375rem] flex-col gap-[1.135rem] lg:gap-6 pb-[1.135rem] lg:pb-[1.5rem] w-full"
			>
				...
				<UiInput
					hide-error
					class-name="w-full bg-transparent"
					name="callBackText"
					placeholder="Комментарий"
				/>
				<UiFileInput
					placeholder="Загрузить проект"
					name="callBackFile"
				/>
			</div>
			<UiButton
				class="w-full mb-2.5 lg:mb-3.5 lg:max-w-[26.375rem]"
				title="Отправить"
				type="submit"
				variant="dark"
				@click="sendForm"
			/>
			<FormHelperLink
				class="text-center lg:max-w-[26.375rem] block w-full"
			/>
		</div>
	</section>
</template>
