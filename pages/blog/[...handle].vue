...
import ArticleContent from "~/components/blocks/blog/ArticleContent.vue";
...
const { data } = await useAsyncData(() =>
	getItems<IIdeaItem>({
		...
		params: {
			...
			fields: ["*", "contentBlocks.*", "tags.blogTags_id.*"],
		},
	})
);
const { data: seems } = await useAsyncData(() =>
	getItems<IIdeaItem>({
		collection: "projects",
		params: {
			filter: {
...
const article = computed<IIdeaItem | null>(() =>
	...
	<div class="flex-1">
		...
		<div class="py-8 px-4 lg:py-[3.25rem] page">
			<div v-if="article"
				...
				<div v-for="tag of article.tags" :key="tag.blogTags_id.id"
					class="bg-system-gray px-8 lg:px-10 py-2 lg:py-4 rounded-full">
					{{ tag.blogTags_id.title }}
				</div>
				<div>
					{{ new Date(article.date_created).toLocaleDateString() }}
				</div>
			</div>
			<template v-for="item of article?.contentBlocks" :key="item.id">
